...
for i in $@ ; do
    ...
    dof=6       ## Default
    UseJacobian=false            #true or false (NOTE: the jacobian option only applies the jacobian of the distortion corrections to the fMRI data, and NOT from the nonlinear T1 to template registration)
...
#           done
    ...
    if [[ $UseJacobian == "true" ]] ; then
        echo "${i} Resting Functional Pre-Register : Applying Jacobian correction to scout image"
        ...
        echo "${i} Resting Functional Pre-Register : Not Applying Jacobian Correction to Scout Image"
    fi
    ...
    if [ ! -f ${DCFolder}/PhaseOne_gdc_dc_unbias.nii.gz ] || [ ! -f ${DCFolder}/PhaseTwo_gdc_dc.nii.gz ] ; then
    ...
    if [ ! -e ${ScoutOutput}.nii.gz ] ; then
        ...
        if [ ! -e ${onestep}/postvols_mcflirt/vol0249_mask.nii.gz ]; then
            ...
            for ((k=0; k < $NumFrames; k++)); do
                ...
                ###Add stuff for RMS###
                ${FSLDIR}/bin/convertwarp --relout --rel --ref=${onestep}/prevols_mcflirt/vol${vnum}.nii.gz --warp1=${GradientDistortionField} --postmat=${MotionMatrixFolder_mcflirt}/${MotionMatrixPrefix}${vnum} --out=${MotionMatrixFolder_mcflirt}/${MotionMatrixPrefix}${vnum}_gdc_warp.nii.gz
                ${FSLDIR}/bin/convertwarp --relout --rel --ref=${onestep}/${T1wImageFile2}.${FinalfMRIResolution} --warp1=${MotionMatrixFolder_mcflirt}/${MotionMatrixPrefix}${vnum}_gdc_warp.nii.gz --warp2=${OutputTransform2} --out=${MotionMatrixFolder_mcflirt}/${MotionMatrixPrefix}${vnum}_all_warp.nii.gz
                ${FSLDIR}/bin/fslmaths ${onestep}/prevols_mcflirt/vol${vnum}.nii.gz -mul 0 -add 1 ${onestep}/prevols_mcflirt/vol${vnum}_mask.nii.gz
