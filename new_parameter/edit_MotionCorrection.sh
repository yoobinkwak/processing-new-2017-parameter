...
# --------------------------------------------------------------------------------
HCPPIPEDIR_Global=/home/yoobinkwak/preproc_BCS_001_002/Pipelines-master/global/scripts      ##line 7 added to run for the time being
...
out=${WorkingDirectory}/${OutputMotionRegressors}.txt
cat $in | sed s/"  "/" "/g > $out
i=1
while [ $i -le 6 ] ; do
...
awk -f ${HCPPIPEDIR_Global}/mtrendout.awk $out > ${WorkingDirectory}/${OutputMotionRegressors}_dt.txt
