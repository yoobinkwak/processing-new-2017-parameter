...
#                fi
        ...
        if [ ! -f ${rawdir}/series_index.txt ] && [ ! -f ${eddydir}/series_index.txt ]
        then
...
#### Move files to appropriate directories (basic_preproc.sh) ####
    ...
    then
        ...
        mv ${rawdir}/acqparams.txt ${topupdir}/acqparams.txt
...
#### Run Eddy (from run_eddy.sh) #### 
    if [ ! -f ${eddydir}/eddy_unwarped_images* ]
    then
...
#        eddy --imain=${eddydir}/Pos_Neg --mask=${eddydir}/nodif_brain_mask --index=${eddydir}/series_index.txt --acqp=${eddydir}/acqparams.txt --bvecs=${eddydir}/Pos_Neg.bvecs --bvals=${eddydir}/Pos_Neg.bvals --topup=${topupdir}/topup_Pos_Neg_b0 --out=${eddydir}/eddy_unwarped_images --flm=quadratic 
    else
