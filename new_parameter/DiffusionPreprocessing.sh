...
#### Extract b0 and write text files for Topup and Eddy (basic_preproc.sh) ####
    ...
    do
        ...
        elif [ $check_index -gt $correct_index ]
        then
            echo ERROR : ${i} need to re-process sereis_index.txt 
        elif [ $check_index -lt $correct_index ]
        then
            sesdimt=`${FSLDIR}/bin/fslval ${entry} dim4`    
            for (( j=0; j<${sesdimt}; j++ ))
            do                
...
#### Post Eddy ####
    datadir=${preproc_diff}/data
    if [ ! -d ${datadir} ]
    then
        mkdir ${datadir}
    fi
    GdCoeffs=NONE       ##path to file containing coefficients that describe spatial variations of the scanner gradients. Use --gdcoeffs=NONE if not available.
    CombineDataFlag=2
    ...
    if [ ! -f ${datadir}/data.nii.gz ]
    then
        bash ${HCPPIPEDIR}/DiffusionPreprocessing/scripts/edit_eddy_postproc.sh ${preproc_diff} ${GdCoeffs} ${CombineDataFlag}
    else
        echo ${i} Post Eddy processing completed
    fi

#### Diffusion to strcutural registraion ####
