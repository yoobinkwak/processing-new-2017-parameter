...
#        ${FSLDIR}/bin/fslmaths ${distortion}/TopupField -mul 6.283 ${distortion}/TopupField
    ...
    fi      ## TopupPreprocessingAll.sh done
    ...
    log_Msg "apply Jacobian correction to scout image"      ## if [[ $UseJacobian == "true" ]]
    ${FSLDIR}/bin/fslmaths ${distortion}/SBRef_undistorted -mul ${distortion}/Jacobian.nii.gz ${distortion}/SBRef_jac_undistorted

#### register undistorted scout image to T1w (from DistortionCorrectionAndEPIToT1wReg_FLIRTBBRAndFreeSurferBBRbased.sh and epi_reg) ####
    ...
    vrefhead=${i}/T1*/20*       ##T1wImage
    vrefbrain=${i}/Preprocess_Structure/PreFreesurfer/BiasField/T1wRestoredBrainImage.nii.gz        ##T1wBrainImage (bias-corrected, brain extrated T1W)
    ...
    if [ ! -f ${vout}.nii.gz ]
    then
        log_Msg "register undistorted scout image to T1w"       ## lines 97~224 from epi_reg_dof
        ...
        then
            ...
            echo ${i} WM edge for visualisation created
        fi
    ...
    #### generate combined warpfields and spline interpolated images and apply bias field correction ####
    if [ ! -f ${vout}_warp.nii.gz ]
    then
        log_Msg "generate combined warpfields and spline interpolated images and apply bias field correction" 
        ${FSLDIR}/bin/convertwarp --relout --rel -r ${vrefhead} --warp1=${distortion}/WarpField.nii.gz --postmat=${vout}.mat -o ${vout}_warp
        ${FSLDIR}/bin/applywarp --rel --interp=spline -i ${distortion}/Jacobian.nii.gz -r ${vrefhead} --premat=${vout}.mat -o ${epi_reg_dir}/Jacobian2T1w.nii.gz
    else
        echo ${i} combined warpfields and spline interpolated images generated and bias field correction applied 
    fi
    #### 1-step resample from input scout - NOTE: no longer includes jacobian correction, if specified
    if [ ! -f ${vout}_1step.nii.gz ] 
    then
        ${FSLDIR}/bin/applywarp --rel --interp=spline -i ${REST_SBREF_LR} -r ${vrefhead} -w ${vout}_warp -o ${vout}_1step
    else
        echo ${i} 1-step resampled from SBRef
    fi

    #### resample phase images to T1w space ####
    if [ ! -f ${epi_reg_dir}/SBRef_dc.nii.gz ]
    then
        Files="PhaseOne_gdc_dc PhaseTwo_gdc_dc SBRef_dc"
        for File in ${Files}
        do
            log_Msg "apply Jacobian correction to scout image"      ## if [[ $UseJacobian == "true" ]]
            ${FSLDIR}/bin/applywarp --interp=spline -i "${distortion}/${File}_jac" -r ${i}/Preprocess_Structure/PreFreesurfer/T1w/T2_acpc_dc.nii.gz --premat=${epi_reg_dir}/fMRI2str.mat -o ${epi_reg_dir}/${File}     ##${i}/Preprocess_Structure/PreFreesurfer/T1w/T2w_acpc_dc.nii.gz is the same as ${i}/Preprocess_Structure/PreFreesurfer/T2toT1_Register/T2w2T1w.nii.gz
            ${FSLDIR}/bin/applywarp --interp=spline -i "${distortion}/${File}" -r ${i}/Preprocess_Structure/PreFreesurfer/T1w/T2_acpc_dc.nii.gz --premat=${epi_reg_dir}/fMRI2str.mat -o ${epi_reg_dir}/${File}     ##${i}/Preprocess_Structure/PreFreesurfer/T1w/T2w_acpc_dc.nii.gz is the same as ${i}/Preprocess_Structure/PreFreesurfer/T2toT1_Register/T2w2T1w.nii.gz
        done
    else
        echo ${i} phase images resample to T1w space
    fi
