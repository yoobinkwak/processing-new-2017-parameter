...
#### recon-all ####
    ...
    if [ ! -f ${fs_brainmask} ]
    then
        #### generate brain mask (generates talairach_with_skull.log at SUBJECTS_DIR ####
        mri_convert ${T1_brain_1mm} ${FS}/${i}/mri/brainmask.mgz --conform
        mri_em_register -mask ${FS}/${i}/mri/brainmask.mgz ${FS}/${i}/mri/nu.mgz $FREESURFER_HOME/average/RB_all_withskull_2016-05-10.vc700.gca ${FS}/${i}/mri/transforms/talairach_with_skull.lta
        mri_watershed -T1 -brain_atlas $FREESURFER_HOME/average/RB_all_withskull_2016-05-10.vc700.gca ${FS}/${i}/mri/transforms/talairach_with_skull.lta ${FS}/${i}/mri/T1.mgz ${FS}/${i}/mri/brainmask.auto.mgz 
        cp ${FS}/${i}/mri/brainmask.auto.mgz ${FS}/${i}/mri/brainmask.mgz 
    else 
        echo ${i} FS brainmask made
    fi

    fs_white_lh=${FS}/${i}/mri/lh.white 
    if [ ! -f ${fs_white_lh} ]
    then
